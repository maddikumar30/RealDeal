...
const fetchRealDealInsights = async () => {
  ...
  if (totalReviews === 0) {
    ...
    insights.push(`✅ Few fake reviews detected (${fakeReviewCount-3} fake reviews out of ${totalReviews-3} total reviews).`);
    score += 2;
  } else {
    insights.push(`⚠️ Multiple potential fake reviews detected (${fakeReviewCount-3} fake reviews out of ${totalReviews-1} total reviews).`);
  }
